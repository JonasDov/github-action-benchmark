...
function getIcon(difference, deviation, percentageThreshold, absoluteThreshold) {
    if (difference === 0) {
        return '🟰';
    }
...
function buildComment(benchName, curSuite, prevSuite, config) {
    const { prCommentAbsoluteThreshold, prCommentPercentageThreshold } = config !== null && config !== void 0 ? config : {
    ...
    for (const current of curSuite.benches) {
        ...
        if (prev) {
            ...
            const statusIcon = getIcon(difference, deviation, prCommentPercentageThreshold, prCommentAbsoluteThreshold);
            line = `| \`${current.name}\` | ${strVal(current)} | ${strVal(prev)} | \`${floatStr(deviation)}%\` | ${statusIcon} |`;
        }
        else {
...
async function leaveComment(commitId, body, commentId, token, commentOnPullRequest) {
    core.debug('Sending comment:\n' + body);
    const repoMetadata = getCurrentRepoMetadata();
    const pr = github.context.payload.pull_request;
    return await ((pr === null || pr === void 0 ? void 0 : pr.number) && commentOnPullRequest
...
async function handleComment(benchName, curSuite, prevSuite, config) {
    ...
    if (!githubToken) {
        throw new Error("'comment-always' or 'comment-on-pull-request' inputs are set but 'github-token' input is not set");
    }
    ...
    if (commentOnPullRequest) {
        if (pr === null || pr === void 0 ? void 0 : pr.number) {
            return await (0, leavePRComment_1.leavePRComment)(repoMetadata.owner.login, repoMetadata.name, pr.number, body, commentId, githubToken);
        }
        if (!commentAlways) {
            return;
        }
    }
    return (0, leaveCommitComment_1.leaveCommitComment)(repoMetadata.owner.login, repoMetadata.name, commitId, body, commentId, githubToken);
}
